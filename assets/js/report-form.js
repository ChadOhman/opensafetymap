...
async function handlePhotoSelect(files) {
  ...
  for (const file of toUpload) {
    try {
      ...
      const res = await fetch('/api/reports/upload.php', { method: 'POST', body: fd });
      const json = await res.json();
      if (json.success && json.data) {
        uploadedPhotos.push({ url: json.data.url, upload_token: json.data.upload_token });
...
async function handleVideoSelect(file) {
  ...
  try {
    ...
    const json = await res.json();
    if (json.success && json.data) {
      uploadedVideo = { url: json.data.url, upload_token: json.data.upload_token };
